...
import com.miotlink.commom.network.mlcc.agent.Make;
import com.miotlink.commom.network.mlcc.agent.Parse;
import com.miotlink.common.network.mlcc.pojo.response.RespBaseAck;
import com.miotlink.common.network.mlcc.pojo.response.RespFc_completeAck;
...
import com.miotlink.common.network.mlcc.pojo.response.RespSetWifiAck;
import com.miotlink.common.network.mlcc.pojo.response.RespSmartConnectedAck;


public class MiotlinkTools implements MiotlinkUdpSocket.IReceiver{
	...
	public static boolean MiotSearch() {
		...
		return MiotlinkTools.sendUdp(MiotlinkTools.cuUdpSocket, "255.255.255.255",
				MiotlinkTools.Search_Pu_Port, 1, makeSearch);
	}
	...
	/**
	 * 1926731071
	 * 青岛服务器   MlDip 114*256*256*256+215*256*256+149*256+63
	 * 电信服务器 MlDipI 122.225.196.132 
	 * 
	 * @param map
	 *            www.51miaomiao.com 
	 * 杭州服务器 112.124.115.125  1887204221
	 *        192.168.10.107
	 *        
	 *        192.168.10.200
	 * @return
	...
	public static boolean MiotFirstConfig(Map<String, String> map) {
		...
		return MiotlinkTools.sendUdp(MiotlinkTools.cuUdpSocket, "255.255.255.255",
				MiotlinkTools.Search_Pu_Port, 1, makeFirst);
	}
	...
	public static boolean MiotFirst4004_AP_Config(String string) {
		...
		try {
			...
			return MiotlinkTools.sendUdp(MiotlinkTools.cuUdpSocket, "255.255.255.255",
					MiotlinkTools.Search_Pu_Port, 1, bs);
		} catch (UnsupportedEncodingException e) {
	...
	public static boolean sendUdp(MiotlinkUdpSocket udpSocket, String ip, int port,
	...
	public void onReceive(int localPort, String host, int port, byte[] src,
		...
		if (localPort == Search_Cu_Port) {
			...
			if (respBaseAck instanceof RespSmartConnectedAck) {
				m.what = FC_FIRST_CONFIG;
				m.obj = map;
				if (fcfirstConfig != null)
					fcfirstConfig.sendMessage(m);
			} else if (respBaseAck instanceof RespFc_completeAck) {
