...
public class NoFormatConsts {
	...
	public static List<FirstData> getInstance(String uart) {
	 ...
	 static void addSmartConfigInfo(){
		Utils.Consts.smartList.clear();
		FirstData firstData = new FirstData("1",
				"CodeName=fc_ml_platform&pf_url=" + SDK_PLATFORM
						+ "&pf_port=28001&pf_ip1=" + SDK_PLATFORM_IP
						+ "&pf_ip2=122.225.196.132&", "fc_ml_platform_ack",
				"0");
		Utils.Consts.smartList.add(firstData);
	}

	public static void addSmartConfigHF(String platfrom,int port,String uart){
		Utils.Consts.smartList.clear();
		...
		Utils.Consts.smartList.add(new FirstData("1", "CodeName=SetWifi&MlPort=" + port
				+ "&MLinkIp=" + platfrom
				+ "&MlDip=1992180694&MlDipI=1887204221&UartInfo=0`" + uart
				+ "`8`0`256`100", "SetWifiAck", "0"));
	}

	public static void setPlatform(String address,int port){
		if (Utils.Consts.smartList!=null&&Utils.Consts.smartList.size()==0){
			addSmartConfigInfo();
		}
		if (TextUtils.isEmpty(address)&&port==0){
			return;
		}
		String platfrom = "CodeName=fc_user_platform&service=" + 2
				+ "&pf_url=" + address + "&pf_port=" + port + "&";
		Utils.Consts.smartList.add(new FirstData((Utils.Consts.smartList.size()+1)+"", platfrom, "fc_user_platform_ack",
				"0"));
	}

	public static void setUart(String uart){
		if (Utils.Consts.smartList!=null&&Utils.Consts.smartList.size()==0){
			addSmartConfigInfo();
		}
		if (TextUtils.isEmpty(uart)){
		...
		String uartData = "CodeName=fc_uart&uart_info=0`" + uart
				+ "`8`0`1`256`100&";
		Utils.Consts.smartList.add(new FirstData((Utils.Consts.smartList.size()+1)+"", uartData, "fc_uart_ack",
				"0"));
	}
