...
public class MiotHFSmartConfig {
	...
	public void hFSmartConfig(int type, String routeName, String password,
		...
		if (type == 3) {
			snifferSmartLinker = SnifferSmartLinker.getInstance();
			try {
				snifferSmartLinker.start(context, password, routeName);
				snifferSmartLinker.setTimeoutPeriod(time);
				((MulticastSmartLinker)snifferSmartLinker).setMixType(MulticastSmartLinker.MIX_TYPE_SMART_LINK_V8);
